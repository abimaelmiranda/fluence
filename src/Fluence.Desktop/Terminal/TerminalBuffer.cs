...
/// <summary>
/// Minimal VT100/ANSI terminal state machine.
/// Parses escape sequences and maintains a character grid.
/// </summary>
public sealed class TerminalBuffer
{
    ...
    /// <summary>
    /// Returns a consistent snapshot of the entire buffer state under a single lock.
    /// Render must use this instead of reading Rows/Columns/Get separately.
    /// </summary>
    ...
    private void HandlePrintable(char ch)
    {
        switch (ch)
        {
            ...
            case '\t':
                ...
                if (_cursorX >= Columns) _cursorX = Columns - 1;
                break;
            default:
                if (ch >= 0x20)
                {
    ...
    private void ClearScreen(int mode)
    {
        ...
        // Do not let shell prompt redraw sequences wipe the visible terminal surface.
        // ClearLine handles prompt edits; full-screen clears need a real scrollback/viewport model.
    }

    private void ClearLine(int mode)
    {
        // mode 0: cursor→end  |  mode 1: start→cursor  |  mode 2: entire line
    ...
    private void ApplySgr(string[] parts)
    {
        for (int i = 0; i < parts.Length; i++)
        {
            ...
            switch (code)
            {
                ...
                // 256-color and truecolor handled minimally
    ...
    private int GetResizeSourceStartRow(int rowsToCopy)
    {
        ...
        var firstContentRow = FirstNonEmptyRow();
        var lastContentRow = LastNonEmptyRow();
        if (firstContentRow < 0 || lastContentRow < 0)
        {
            return Math.Max(0, Rows - rowsToCopy);
        }

        if (lastContentRow < rowsToCopy && _cursorY < rowsToCopy)
        {
            return 0;
        }
    ...
    {
        return (ch.Glyph is ' ' or '\0') && ch.Background == Colors.Transparent;
    }
    ...
    // Standard 16 ANSI colors (xterm palette)
    ...
    [
        ...
        Color.FromRgb(85,  85,  85),   // 8 Bright Black (grey)
