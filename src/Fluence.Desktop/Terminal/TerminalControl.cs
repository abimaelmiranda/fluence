...
/// <summary>
/// Custom Avalonia control that renders a TerminalBuffer grid and forwards keyboard input.
/// </summary>
public sealed class TerminalControl : Control
{
    ...
    /// <summary>
    /// Called by the view after subscribing to Resized to trigger the initial resize,
    /// since ArrangeOverride may have already fired before the handler was registered.
    /// </summary>
    public void RequestInitialResize()
    {
        ...
        {
            // Bounds not ready — reset guard so the next ArrangeOverride fires Resized
            _lastCols = 0;
            _lastRows = 0;
            InvalidateArrange();
        }
    ...
    public override void Render(DrawingContext ctx)
    {
        ...
        // Take an atomic snapshot — avoids IndexOutOfRangeException if Resize races with Render.
        ...
        for (int row = 0; row < snap.Rows; row++)
        {
            for (int col = 0; col < snap.Columns; col++)
            {
                ...
                {
                    ctx.FillRectangle(
                        new SolidColorBrush(ch.Background),
